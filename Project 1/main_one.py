## Практическое задание № 1. Линейная регрессия (с одной переменной)
...
X = np.array([data[:, 0]]); X = np.transpose(X); X = np.concatenate((np.ones((m, 1)), X), axis = 1)
y = np.array([data[:, 1]]); y = np.transpose(y)
